...
公開預覽標為 `noindex`，無自然搜尋或 AI 回答引用的成效資料；本例只能展示
互動、靜態圖文備援與頁面實作，不能作為 SEO／AEO 排名實績。

## 素材與實作

- `assets/final.png`：完成稿原圖，1024×1536。
- `assets/linework.png`：以完成稿為參考編輯的白描原圖。
- `assets/underpaint.png`：同構圖的淡彩原圖。
- `assets/studio.png`：筆、墨、顏料與絹面的畫案情境原圖。
- `public/assets/*.webp`：網站實際載入的壓縮版本，完成稿另有 720px 首屏變體。
  原始 PNG 保留在公開目錄外供檢視與重新輸出；執行 `python optimize_assets.py` 可重建 WebP，
  需要 Pillow。
...
2026-09-23 本機檢查：完成稿原圖 2,833,319 bytes，完整 WebP 340,300 bytes，
720px WebP 178,322 bytes。修改前公開頁的模擬手機 Lighthouse LCP 約 2.11 秒，
其 LCP 元素是首屏 H1；壓縮圖片是減少傳輸量，不能拿來宣稱改善了 LCP。
