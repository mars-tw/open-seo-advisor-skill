...
再依自己的品牌設計；這是虛構品牌的故事首頁與獨立製作說明頁，不提供預約或交易。
...
兩個網址展示敘事與互動，不是 SEO 成效案例，也不能證明已收錄、符合 Core Web Vitals
或取得 AI 搜尋引用。要建可對外營運的站，仍須完成逐頁內容、效能與上線後驗證。
...
1. **需求與搜尋架構**：建立 `site-plan.md`，記錄站型、受眾、CTA、功能、參考觀察、
   主機、未知事項及已有授權。先對照真實搜尋需求與各頁任務，決定是否需要獨立 URL；
   商品、分類、政策等資訊不能靠首頁錨點冒充獨立頁。
...
5. **搜尋與功能**：讀 [SEO／AEO](docs/website-search.md)，逐頁核對獨特且準確的 title／
   meta description、清楚的主標與有意義的章節標題、可讀 HTML、內部連結及索引設定；
   量測首屏 LCP 並按證據修復。有商品、表單或結帳時讀[商務流程](docs/website-commerce.md)。
...
- 一頁式敘事可以服務單一明確目的；若要承接不同商品、服務或查詢意圖，按實際需求
  建立可直接開啟的頁面。H2 不設固定數量，也不把標題跳級誤稱為 Google 排名懲罰；
  標題層級應讓讀者與輔助技術容易理解。
...
- 品牌名稱查詢或自問自答的單次截圖不能證明 AEO 成效。若使用者要追蹤成效，分開記錄
  非品牌需求查詢、Search Console 資料與回答引擎觀察樣本；無資料就寫尚未驗證。
