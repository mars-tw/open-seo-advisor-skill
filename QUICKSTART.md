...
這份指南寫給完全沒有寫過程式、沒有 SEO 背景的人。跟著做，5-10 分鐘內就能拿到
一份看得懂的網站健檢報告。
...
你會看到一個用問答方式引導你的精靈，像這樣：

```text
歡迎使用 Open SEO Advisor
我會幫你檢查一個網站的 SEO 健康狀況，全程用問答方式進行，不需要記任何指令。

請問你想做什麼？
  1 - 掃描一個真實網站，產生 SEO 健檢報告
  2 - 掃描本機資料夾裡的網站原始碼
  3 - 先看一份範例報告（不需要輸入網址）
```
...
輸入 `3` 直接按 Enter，工具會用內建的範例資料產生一份完整報告，讓你先看看
報告長什麼樣子，完全不需要輸入任何網址。
