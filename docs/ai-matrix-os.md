...
- **IRIS（SEO）** → 顧問模式引擎（後續版本接上）
- **MAYA / LUNA / ECHO / CODY（內容類）** → 文章寫手引擎
- **JACK（廣告）** → Meta 廣告引擎
- **PIXEL（產圖）** → 產圖素材引擎
- 其餘沒有專屬引擎的角色（策略/財務/人資/法務/客服/營運…）→ 通用 LLM 引擎

> 目前版本（v0.1.4，Phase 1）：所有角色統一透過 **MockEngine**（免金鑰）或
> **GenericLLMEngine**（有金鑰時）執行，讓整條路由→派工→執行→整合流程能
> 完整運作且免金鑰可試玩。把各角色接到專屬引擎是後續版本的工作
> （見 `docs/roadmap.md`）。
